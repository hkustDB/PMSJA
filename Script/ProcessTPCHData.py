...
def main(argv):
	...
	for opt, arg in opts:
		if opt == '-h':
			...
			primary_relation_file_name = arg
	if model != 0:
		...
		if dataset not in datasets:
			...
			sys.exit()
		cur_path = os.getcwd()
		data_path = cur_path+"/../Data/TPCH/scale_" + dataset + "/"
		relations = ["REGION", "NATION", "SUPPLIER", "CUSTOMER", "PART", "PARTSUPP", "ORDERS", "LINEITEM"]
		primary_relations = []
		if primary_relation_file_name != '':
			primary_relation_file = open(cur_path + "/../Profile/" + primary_relation_file_name,'r')
