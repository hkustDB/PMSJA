...
def ECP(r):
    ...
    with mosek.Env() as env:
        ...
        env.putlicensepath("../Profile/mosek.lic")
        with env.Task(0, 0) as task:
            # Attach a printer to the task
            #task.set_Stream(mosek.streamtype.log, streamprinter)
            #First, deal with variables
            #The type pf constraint for each variable
            bkx = [mosek.boundkey.ra]*N+[mosek.boundkey.lo]*total_M+[mosek.boundkey.lo]*total_contribution_dim+[mosek.boundkey.fx]*N
...
def RunAlgorithm():
    ...
    T_hat = 2*(N-E_r) + LapNoise()*2/(0.45*epsilon) + 2/(0.45*epsilon)*math.log(pow(math.e, epsilon*0.55)/delta)
    Gauss_noise = T_hat *r*math.sqrt(2*math.log(1/temp)) * (1+0.45*epsilon/(4*math.log(1/temp))) / (0.45*epsilon) * np.random.normal(0, 1, d)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"h:I:e:b:d:",["InputPrefix=","epsilon=","beta=","delta="])
    except getopt.GetoptError:
        print("PMSJA.py -I <input file prefix> -e <epsilon(default 4)> -b <beta(default 0.1)> -d <delta>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print("PMSJA.py -I <input file prefix> -e <epsilon(default 4)> -b <beta(default 0.1)> -d <delta>")
